package testTablaHash;

import java.util.Iterator;

import tablaHash.ITablaHash;
...
public class TablaHashTest extends TestCase  {
	...
	public void setupEscenario1(){
		...
		try{
			//se tiene en cuenta para las pruevas de get que 3 elementos se insertaron con la misma llave
	...
	public void testPut()
	{
		...
		//se agrego un elemento identico a uno que ya esta en la tabla,tanto por llave como por valor, no lo dejo agregar
		...
		setupEscenario1_1();
	/*	Iterator<TestClass> iter=tabla.iterator();
		int cont=0;
		while (iter.hasNext()) {
			TestClass este = (TestClass) iter.next();
			cont++;
			System.out.println(este+"  "+cont);
		}*/
		//se imprimen los elementos guardados en la tabla y luego se mira las estadisticas de la misma para saber que estan bien 
	...
	public void testGets(){
		setupEscenario1();
		setupEscenario1_1();
		//se cargan los ecenarios de la ves pasada y se espera que presenten las mismas estadisticas
		assertEquals(20, tabla.darLargo());
		assertEquals(12, tabla.darCantidadElementosIterables());
		assertEquals(13, tabla.darCantidadElementosTabla());	
		//creamos un obejto y metemos la llave con la que se indexo "Hector"
		ClasePrueba prueva= tabla.get(68+"");
		assertTrue("no se rescato el elemento deseado", prueva.getText().equals("Hector"));
		//como salio exitoso para un solo elemento probamos para cuando se indexaron varios con la misma llave
		...
		for (int i = 0; i < grupo.length; i++) {
			...
			try{
			if(i==0)			{
				assertTrue("no se rescato el elemento deseado::"+ este.getText(), este.getText().equals("Adriana"));
			}
			else if(i==1)			{
				assertTrue("no se rescato el elemento deseado::"+ este.getText(),este.getText().equals("Bernardo"));
			}
			else if(i==2){
				assertTrue("no se rescato el elemento deseado::"+ este.getText(),este.getText().equals("Elena"));
			}
			}
			catch (NullPointerException e) {
				//se supone que esto no sucede en este test, es cuando 2 llaves diferentes llegan al mismo espacio
				//es decir se da una colicion, el arreglo de retorno de la tabla lleva huecos vacios pues hace las verificaciones
				//corresponsientes y deja atras a todos los elementos que no coincidan con la llave
				System.out.println("MIERDA!");
			}
