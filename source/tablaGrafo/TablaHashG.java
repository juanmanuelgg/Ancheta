...
public class TablaHashG<K,V extends IVertice<T>,T> implements ItablaHashG<K, IVertice<T>, T> {
	...
	private void rehash() {
		...
		while (iter.hasNext()) {
			PaqueteHash<K, grafo.IVertice<T>> estePaquete = (PaqueteHash<K, grafo.IVertice<T>>) iter.next();
	...
	public IVertice<T> get(K llave) {
		...
		while (iter.hasNext()) {
			PaqueteHash<K, grafo.IVertice<T>> este = (PaqueteHash<K, grafo.IVertice<T>>) iter.next();
	...
	public Iterator<IVertice<T>> iterator() {
		...
		Iterator<PaqueteHash<K,IVertice<T>>> iter= copia.iterator();
		while (iter.hasNext()) {
			PaqueteHash<K, grafo.IVertice<T>> estePAque = iter.next();
